...
def main() -> int:
    ...
    blocked_result = replan_from_position(blocked_graph, start_x, start_y, baseline_target, sigma=0.1, loihi_config=config)
    ...
    blocked_payload = {
        ...
        "blocked_edge_in_new_path": any((a, b) == (edge_u, edge_v) for a, b in zip(blocked_path, blocked_path[1:])),
    ...
    penalized_result = replan_from_position(penalized_graph, start_x, start_y, baseline_target, sigma=0.1, loihi_config=config)
    penalized_payload = {
        "baseline_start": baseline_start,
        "baseline_target": baseline_target,
        "baseline_path": baseline_path,
    ...
    try:
        plot_graph_with_path(G, example_path, str(output_dir / "dynamic_replanning_example.png"), title="dynamic replanning")
    ...
    summary = {
        ...
        "dynamic_success_rate": sum(1 for row in dynamic_rows if row["success"]) / len(dynamic_rows) if dynamic_rows else 0.0,
        "blocked_edge_avoided": not blocked_payload["blocked_edge_in_new_path"] if blocked_payload["success"] else None,
